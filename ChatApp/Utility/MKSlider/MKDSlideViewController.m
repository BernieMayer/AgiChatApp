...
- (instancetype)initWithMainViewController:(UIViewController *)mainViewController;
{
    ...
    if( self )
    {
        ...
        _leftStatusBarStyle = UIStatusBarStyleLightContent;
        _rightStatusBarStyle = UIStatusBarStyleLightContent;
        _slideSpeed = 0.3f;
        _overlapWidth = 52.0f;
    }
    return self;
...
- (void)loadView
{
    ...
    //    // Uncommented on 24-01
    //   [_mainPanelView addGestureRecognizer:self.panGestureRecognizer]; // Uncommented
...
- (void)updatePanelsForCurrentPosition
{
    ...
    CGFloat xOffset = self.mainPanelView.frame.origin.x/3;
    ...
    if( position == MKDSlideViewControllerPositionCenter )
    {
        ...
        else if( xOffset < (dividerPosition-snapThreshold) )
        {
            ...
            [self showRightViewControllerAnimated:YES offset:0];
            if( [self isHandlingStatusBarStyleChanges] )
    ...
    else if( position == MKDSlideViewControllerPositionLeft )
    {
        ...
        {
            // snap to center position
            if(xOffset<_tapOverlayView.frame.size.width/2)
                [self showMainViewControllerAnimated:YES];
            else
                [self showLeftViewControllerAnimated:YES];
    ...
    else if( position == MKDSlideViewControllerPositionRight )
    {
        ...
        else
        {
            ...
            if(rightSideX>_tapOverlayView.frame.size.width/2)
                [self showMainViewControllerAnimated:YES];
            else
                [self showRightViewControllerAnimated:YES offset:0];
...
- (UIView *)tapOverlayView
{
    if( _tapOverlayView == nil )
    {
        ...
        [_tapOverlayView addGestureRecognizer:tapGesture]; // Uncommented
        [tapGesture release];
    }
...
{
    [self showLeftViewControllerAnimated:YES offset:0];
}

- (void)showLeftViewControllerAnimated:(BOOL)animated offset:(int)leftOffSet
{
    ...
    if( animated )
    {
        [UIView animateWithDuration:self.slideSpeed animations:^{
           ...
           // theFrame.origin.x = -theFrame.size.width + self.overlapWidth - leftOffSet;
            theFrame.origin.x = leftOffSet;
            _mainPanelView.backgroundColor = [UIColor blueColor];
...
- (void)showRightViewControllerAnimated:(BOOL)animated offset:(int)rightOffSet
{
    ...
    if( animated )
    {
        [UIView animateWithDuration:self.slideSpeed animations:^{
            CGRect theFrame = self.mainPanelView.frame;
            theFrame.origin.x = -theFrame.size.width + self.overlapWidth - rightOffSet;
            self.mainPanelView.frame = theFrame;
        } completion:^(BOOL finished) {
            [self addTapViewOverlay];
...
- (void)showMainViewControllerAnimated:(BOOL)animated
{
    ...
    [_mainPanelView removeGestureRecognizer:self.panGestureRecognizer];
