...
open class Countries {
    ...
    // MARK: - var and let
    fileprivate static let selectedCountryName = "selectedCountryName"
    fileprivate static let selectedCountryPhoneExtension = "selectedCountryPhoneExtension"
    fileprivate static let selectedCountryisMain = "selectedCountryisMain"
    fileprivate static let selectedCountryCode = "selectedCountryCode"
    fileprivate static let countryIsSelected = "countryIsSelected"
    open static let isSelected = Foundation.UserDefaults.standard.bool(forKey: "countryIsSelected")
    
    // MARK: - functions
    
    open static func getAllCountries() -> [Country] {
        var countries = [Country]()
        Locale.isoRegionCodes.forEach { (isoCode) in
            guard let countryName = (Locale.current as NSLocale).displayName(forKey: NSLocale.Key.countryCode, value: isoCode) else { return }
    ...
    open static func getCountriesByAlphabet() -> [(String, [Country])] {
        let countries = getAllCountries()
        var countryDictionary = [String : [Country]]()
        var countryFirstLetterArray = [String]()
        
        countries.forEach { (country) in
            let letterIndex = country.name.characters.index(country.name.startIndex, offsetBy: 0)
            ...
            if countryFirstLetterArray.count == 249 {
                ...
                countryFirstLetterArray.sort(by: { (firstString, secondString) -> Bool in
        ...
        // sorting
        let returnedCountries = countryDictionary.sorted(by: {$0.0 < $1.0})
        
        return returnedCountries
    }
    
    open static func getCountriesWithPhone() -> [Country] {
        var countries = getCountriesWithPhoneCode()
        countries.sort(by: {$0.name < $1.name})
        return countries
    }
    
    open static func getCountriesByPhoneCode() -> [Country] {
        var countries = getCountriesWithPhoneCode()
        countries.sort(by: {$0.phoneExtension! < $1.phoneExtension})
        return countries
    }
    
    fileprivate static func getCountriesWithPhoneCode() -> [Country] {
        let countries = getAllCountries()
    ...
    open static func getDeviceSelectedCountry() -> Country? {
        var index = 0
        var countryCode = ""
        
        Locale.current.identifier.characters.forEach { (character) in
            index += 1
            if index > 3 {
        ...
        guard let currentCountryName = (Locale.current as NSLocale).displayName(forKey: NSLocale.Key.countryCode, value: countryCode) else { return nil }
    ...
    open static func saveSelectedCountry(_ country: Country) {
        Foundation.UserDefaults.standard.set(country.name, forKey: selectedCountryName)
        if country.phoneExtension != nil {
            Foundation.UserDefaults.standard.set(country.phoneExtension!, forKey: "selectedCountryPhoneExtension")
        }
        if country.isMain != nil {
            Foundation.UserDefaults.standard.set(country.isMain!, forKey: selectedCountryisMain)
        }
        Foundation.UserDefaults.standard.set(country.countryCode, forKey: selectedCountryCode)
        Foundation.UserDefaults.standard.set(true, forKey: countryIsSelected)
        Foundation.UserDefaults.standard.synchronize()
    }
    
    open static func getSelectedCountry() -> Country?  {
        let isSelected = Foundation.UserDefaults.standard.bool(forKey: countryIsSelected)
        if isSelected {
            let countryName = Foundation.UserDefaults.standard.object(forKey: selectedCountryName) as! String
            let countryCode = Foundation.UserDefaults.standard.object(forKey: selectedCountryCode) as! String
            
            guard let phoneExtension = Foundation.UserDefaults.standard.object(forKey: selectedCountryPhoneExtension) as? String else {
                let country = Country(name: countryName, phoneExtension: nil, countryCode: countryCode, isMain: nil)
                return country
            }
            guard let isMain = Foundation.UserDefaults.standard.object(forKey: selectedCountryisMain) as? Bool else {
                let country = Country(name: countryName, phoneExtension: nil, countryCode: countryCode, isMain: nil)
                return country
            }
    ...
    open static func getCountryByPhoneCode(_ countryCode: String) -> Country? {
        var currentCountry: Country!
